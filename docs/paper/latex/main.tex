...
\begin{abstract}
Interactive coding agents share an unexamined default: every model call is synchronous, priced at the realtime rate, with a person nominally waiting. The work such agents do best --- backlog burndown, migrations, test-coverage pushes --- has no one waiting, and every major provider sells a second product for exactly that shape: batch APIs with a 24-hour window at half the price. LazyCode is a coding agent built on the batch tier: a realtime model \emph{plans}, and the plan executes overnight as barrier-synchronized \emph{waves} of self-sufficient batch calls. The architecture is a deliberate transplant of database query processing --- operator algebra, rewrite rules, physical waves, an event-sourced log --- and the paper separates what is implemented (16{,}979 LOC, 379 tests, three-window crash recovery) from what is designed, and from what is vocabulary only. Against a prompt-cached interactive baseline, batch wins on input tokens only when batch rounds and interactive turns satisfy $R < T/5$; if batch and cache discounts stack and waves hit the cache, wide waves relax the condition toward $R < 2T$, shifting the burden from depth-collapse to width-supply. A four-part criterion says when this generalizes into agentic frameworks, and one small upstream protocol change --- deferred \emph{model} requests --- is what blocks durable-execution frameworks from treating model calls like deferred tools.
\end{abstract}
...
Throughout, we distinguish three epistemic layers explicitly: what is \emph{implemented and tested} (16{,}979 LOC, 379 tests), what is \emph{designed with plumbing in place}, and what is \emph{vocabulary only}. Research prototypes usually blur these; a paper about honest economics should not.
...
The system is 16{,}979 lines of Python (8{,}845 in the package, 7{,}281 in tests, 853 in the benchmark harness) with 379 tests. Three findings from auditing our own code, reported because papers rarely do and reviewers always should.

...
GPT-5.4-mini & batch & GPTB1 & GPTB2 & GPTB3 \\
\bottomrule
\end{tabular}
\end{table}
...
Two observations the paper could not have printed before. The
turnaround objection did not materialize at this scale: every batch ---
2 to 50 items --- reached terminal state in 90 seconds to nine minutes
against its 24-hour window (submitted mid-week from IST; no off-peak
shopping). And self-reported agent telemetry is not a receipt: the CLI
...
\paragraph{What this still does not show.} The fan-out tier flatters
the compilation: prompts are small, so the conversation-cache dynamics
of \cref{sec:economics-width} barely register, and
\texttt{rounds\_per\_node} stayed 1 by construction on a shape with no
dependencies. A deep tier (SWE-bench Verified instances, where the
agent's iteration should be worth the most) and the within-wave
cache-stacking measurement remain open, as does a second seed;

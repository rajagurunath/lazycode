...
\emph{Compiled-sync}: the ablation control, not a product mode ---
LazyCode's own compiled single-shot calls executed at the full
synchronous price, so that the compilation effect and the lane discount
can be read separately. \emph{LazyCode}: the same calls as one wave
through the batch lane --- the product. Models are three closed families with a clean 50\% batch lane
...
and stacks on top}: batch cost 50.9\% of sync on Haiku, 51.0\% on
Gemini, and 49.1\% on GPT-5.4-mini for identical request bodies, landing the blended
...
window, while the identically-shaped OpenAI-routed batch sat at 0 of
50 completed for its first 2.4 hours and finalized at 3.5 hours --- a
$25\times$ spread between providers on one workload. The 24-hour window
is the contract;
...
\Cref{tab:headtohead} puts the two tiers side by side.

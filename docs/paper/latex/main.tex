...
\Cref{tab:rules} lists the ten rewrite rules with their database lineage. Two ship today; the table's Status column is load-bearing (\cref{sec:implementation}).
...
\midrule
R1 LocalPushdown & predicate pushdown & \textbf{shipped} & \textsc{Explore} answerable by local tooling (ripgrep, AST outline) executes free, no LLM round \\
